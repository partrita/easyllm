...
def stream_chat_request(client, prompt, stop, gen_kwargs, model):
    ...
    raise NotImplementedError("SageMaker는 아직 스트리밍 요청을 지원하지 않습니다.")
...
class ChatCompletion:
    ...
    def create(
        ...
        if prompt_builder is None:
            logger.warn(
                "sagemaker.prompt_builder가 설정되지 않았습니다. "
                "기본 프롬프트 빌더를 사용합니다. "
                "사용자 지정 프롬프트 빌더를 사용하려면 sagemaker.prompt_builder를 메시지 목록을 가져와 문자열을 반환하는 함수로 설정하세요. "
